...
import { CurrentWeather, CurrentWeatherApiResponse, currentWeatherApiResponsSchema, fetchWeatherData } from "./weatherapi";
...
it("should convert API response", async () => {
  ...
  }).replyOnce(200, SAMPLE_API_RESPONSE)
...
it("should throw error when response is not 200 ", async () => {
  ...
  }).replyOnce(400, SAMPLE_API_RESPONSE)

  await expect(fetchWeatherData(axios, WEATHER_API_URL, "0.0", "0.0")).rejects.toThrow()
})

it("should throw error when response changes ", async () => {
  const httpClient = new MockAdapter(axios)
  const WEATHER_API_URL = "https://api.open-meteo.com/v1/forecast"
...
it("gets low temp", () => {
  const response: CurrentWeatherApiResponse = currentWeatherApiResponsSchema.parse(SAMPLE_API_RESPONSE)
